...
async function validateExpectations() {
    ...
    if (!(req instanceof Set)) {
        throw new Error("requiredExpectations should be of type Set");
    }
...
function validateCreateRequest() {
    ...
    if (
        (typeof req.id !== "string" && !(req.id instanceof ArrayBuffer)) &&
        (typeof req.rawId !== "string" && !(req.rawId instanceof ArrayBuffer))
    ) {
        throw new TypeError("expected 'id' or 'rawId' field of request to be String or ArrayBuffer, got rawId " + typeof req.rawId + " and id " + typeof req.id);
    }
